...
if(isset($_GET['liga_id'])){
    $team = $conn->prepare('SELECT * FROM team WHERE id_liga = '.$_GET['liga_id']);
}else{
...
<body>
        <div style="margin: 8vh 3vw; display: flex; flex-wrap: wrap; justify-content: center;">
            <!-- <div class="card-grid-space"> -->
                <!-- <div class="num"></div> -->
                <?php    
                foreach($result3 as $row){
                    $imageData = $row['gambar_team'];
                ?>
                <a href="d_player.php?team_id=<?=$row['id']?>" class="card">
                    <img src="<?php displayImageById($conn, $row['id']); ?>" alt="">
                    <div class="content">
                        <h1><?php echo $row['nama_team']?></h1>
                        <div class="tags">
                            <p class="tag">Team</p>
                        </div>
                    </div>
